...
void OmpStructureChecker::Enter(const parser::ExecutionPartConstruct &x) {
  ...
  bool haveTargetInfo{!context_.targetTriple().empty()};

  for (const MetadirectiveLoopVariant &variant : variants) {
    const parser::OmpDirectiveSpecification *spec{variant.spec};
    // Skip variants that can never be selected on this compilation target
    // (their device or implementation selectors do not match) so that their
    // associated loop is not diagnosed. Variants with no selector
    // (OTHERWISE/DEFAULT) or with a selector feature that cannot be evaluated
    // yet are always validated.
    if (haveTargetInfo && variant.selector &&
        FindUnsupportedSelectorFeature(*variant.selector, context_) ==
            UnsupportedSelectorFeature::None) {
      llvm::omp::VariantMatchInfo vmi;
      (void)MakeVariantMatchInfo(vmi, *variant.selector, context_);
